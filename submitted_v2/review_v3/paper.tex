...
Alternatively, \sihui{Mach-Zehnder} interferometers can also be employed as building blocks instead of beamsplitters and phase-shifters \cite{bib:Carolan15,bib:Englert2001, bib:Clements16}. \sihui{Such a decomposition and its implementation on an integrated photonic chip are shown in Fig.~\ref{fig:Carolan}. The scheme deployed by Clements {\it et al.}~\cite{bib:Clements16} requires only half the optical depth of Reck {\it et al.} and is robust to optical losses. A different route was taken by de Guise et al.~\cite{bib:deguise18} following a technique that decomposes a $n \times n$ unitary matrix into a beamsplitter sandwiched by two $(n-1) \times (n-1)$ unitary matrices. The technique can be used recursively to an elegant sequence of adjacent beamsplitters. This could be good experimentally for hard coded photonic circuits because it is difficult to mix non-adjacent modes there. The method of Clements et al.~share this advantage, but that of de Guise et al.~has an added advantage, that is, it uses fewer infinitesimal generators to get the same $n \times n$ unitary matrix. When the size of $n$ gets large and/or  the number of photons in the system is also large, the size of the full Hilbert space becomes large and a decomposition that uses fewer generators becomes obviously advantageous.} Later, it was shown by Bouland and Aaronson \cite{bib:Bouland2014} that any nontrivial beamsplitter, that does more than permuting modes or adding phases to them, is universal for linear optics. However, they do not provide an explicit construction for arbitrary unitaries.
...
%\end{figure}
 ...
 As discussed in Section \ref{sec:bosonsampling}, the resources needed for Boson-sampling are readily available, but executes a task that is believed to be hard classically. Surprisingly, an entanglement between a large number of path labels is possible with single photons in a passive linear optical device. Motes {\it et al.~}harnessed this so-called number-path entanglement for quantum metrology without the use of entangled states \cite{bib:Motes15_LOQM,bib:Olsen2017}. Here, the phase shift to be estimated, $\varphi$, manifests itself as a linear phase gradient across $N$ modes (See Fig.~\ref{fig:MORDOR}). By straddling these modes among a quantum Fourier transform and its conjugate operation, it is possible to achieve $\Delta \varphi=\mathcal{O}(N^{-3/2})$ using a coincidence measurement with just a string of $N$ single photons, for up to at least $N=25$ modes. \sihui{Shortly after its construction, this scheme was realized experimentally \cite{bib:Zu-En2017}. The quantum Cram\'{e}r-Rao bound of estimating $\varphi$ in such interferometers was also analytically computed \cite{bib:Chenglong2017}. Also making use of entanglement, in this case, that of continuous variable multipartite entanglement, Zhuang et al.~showed that Heisenberg scaling in $N$ can be achieved when sensing a small phase shift that is distributed across $N$ modes \cite{ZZS18}.
 }
...
%Without the use of repeaters, it has been shown that the rate transmission of a 
%channel with loss without entanglement assistance has a linear fallout 
%\cite{bib:TGW14}, and its bound is known as the Takeoka-Guha-Wilde (TGW) bound. 
%The minimal resources required to exceed the TGW bound is a subject of active 
%research, and so far it has been shown that the performance of the repeater 
%schemes of Ewert-Bergmann-van Loock \cite{bib:Ewert16}, and Sinclair et al. 
%\cite{bib:Sinclair14} are able to exceed the TGW bound. 
Without the use of repeaters it has been shown that the asymptotic rate of transmission of a channel with loss without entanglement assistance has a linear fallout according to the Takeoka-Guha-Wilde (TGW) bound \cite{bib:TGW14} and the tighter Pirandola-Laurenza-Ottaviani-Banchi (PLOB) bound \cite{PLOB17}. In particular, the PLOB bound coincides with the best known lower bound \cite{PGBL09} and establishes the two-way quantum capacity of the lossy channel. PLOB showed that if the transmission rate is above the PLOB bound, the error will not vanish asymptotically. These results were refined by Wilde, Tomamichel and Berta who showed that with a rate above the PLOB bound one will decode wrongly with certainty, and also derived non-asymptotic bounds that are applicable when the channel is used only a finite number of times \cite{WTB17}. Others have generalized the TGW bound for arbitrary network topology \cite{AML16}. The minimal resources required to exceed the linear fallout is a subject of active research, and so far it has been beaten by the repeater schemes of Ewert-Bergmann-van Loock \cite{bib:Ewert16}, and Sinclair et al.~\cite{bib:Sinclair14}.
...
\sihui{We thank Hubert de Guise, Stefano Pirandola, Marco Tomamichel, and Quntao Zhuang for useful comments.} PPR is funded by an ARC Future Fellowship (project FT160100397). This research was supported in part by the Singapore National Research Foundation under NRF Award No.~NRF-NRFF2013-01. ST acknowledges support from the Air Force Office of Scientific Research under AOARD grant FA2386-15-1-4082 \sihui{and FA2386-18-1-4003. ST conducted part of this writing while she was a Guest Researcher at the Niels Bohr International Academy.}
