...
\begin{abstract}
%%%
Linear optics has seen a resurgence owing to its miniaturisation on-chip, and increase in production efficiency and quality of single photons. Time-bin encodings have also become feasible owing to fresh architecture, and processing capabilities. Theoretical efforts to find useful application beyond a full universal quantum computation has also bore fruit. A sampling task, known as {\sc bosonsampling} that is easily implemented using linear optics but is strongly believed to be a hard for a classical computer, has emerged. 
%%%%
\end{abstract}
...
The advancements in linear optics discussed in Section \ref{sec:expt}, quantum computation using linear optics have opened up avenues for implementation. One of the key requirements is nonlinear couplings between the different optical modes. Photons do not naturally interact with one another so some careful engineering is needed to achieve this coupling. Methods used include measurements and feedforwarding \cite{bib:KLM01}, and photon-atom interactions \cite{bib:Brod16}. All models for LOQC also require non-deterministic operations, which succeeds only sometime and have to be repeated. This can be challenging given the limitations of the optical systems. Comprehensive reviews have been written previously to cover this topic \cite{bib:JW12,bib:Kok05}, hence we will focus on recent advances.


 Integrated photonic circuits has greatly improved the feasibility of LOQC. Their small size and interferometric stability enable the requisite quantum interference to happen in certain LOQC settings. Programmable circuits that are reusable have also emerged \cite{bib:Metcalf14,bib:Carolan15}. This saves resources and lead time for producing new circuits. Combining these chips with existing higher-efficiency sources and detectors will expand their capabilities. Loss can be mitigated by moving all components onto the same chip as the linear optical circuit \cite{bib:Sprengers11, bib:Silverstone14}.  What remains is to have fast feedforward also on chip. This might pose the greatest challenge yet for universal LOQC. Only a handful of experiments have been able to implement adaptive feedforward for the purposes of LOQC \cite{bib:Prevedel07,bib:Xiao-Song12,bib:Mikova12,bib:Zhao14}. The bottleneck is in the speed of modulation in the feedforward circuitry.
